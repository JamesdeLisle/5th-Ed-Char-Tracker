...
def dialogue(stat):
    ...
    if stat.returnType() == 'int': 
        ...
        stat = dispDie(stat,indentation,stdscr)
...
def dispDie(stat,indentation,stdscr):
    ...
    error_ypos = 4
    dice = [4,6,8,10,12,20]
    statement = '%s. What kind of dice is it [d4,d6,d8,d10,d12,d20]: ' % (stat.returnStatement())
    ...
    go_flag = True 
    error_statement = ''
    statement = '%s. How many dice: ' % (stat.returnStatement())

    stat.dice_type = dice_type
    ...
    while go_flag:
        ...
        stdscr.addstr(question_ypos,indentation,'%s' % (statement))
        stdscr.refresh()
        dice_num = stdscr.getstr(question_ypos,indentation+len(statement)+2)
